...
### Server Side (Covid19.Server)

#### Models
- `CovidDataPoint`: Combined data model with fields for ProvinceState, CountryRegion, Lat, Long, Date, Confirmed, Deaths, Recovered.
- `CovidConfirmedCase`: Model for confirmed cases.
- `CovidDeathCase`: Model for death cases.
- `CovidRecoverCase`: Model for recovered cases.
- `CovidDailyReport`: Model for daily reports.

#### Controllers
- `CovidConfirmedController`: Handles OData queries for confirmed cases.
- `CovidDataController`: Handles combined data points.
- `CovidDeathController`: Handles death cases.
- `CovidRecoverController`: Handles recovered cases.
- `DailyReportsController`: Handles daily reports.
- `SeedController`: Provides endpoint to seed database from CSV files.

#### Services
- `CovidConfirmService`: Handles seeding confirmed cases from CSV.
- `CovidDeathService`: Handles seeding death cases.
- `CovidRecoverService`: Handles seeding recovered cases.
- `CovidDataService`: Combines data from multiple sources.
- `DailyReportService`: Handles daily report seeding.

#### Program.cs
- Configures OData with support for select, filter, orderby, count, expand.
- Sets up Entity Framework with PostgreSQL.
- Enables CORS for the React app.
- Registers services and controllers.

### Client Side (covid19.client)

#### App.tsx
- Main component that renders the map and treemap visualizations.
- Fetches data from OData endpoints.
- Uses react-simple-maps for world map and recharts for treemap.
- Implements progressive loading with progress indicators.

#### covid.ts
- Defines TypeScript interfaces for CovidData and CountryData.

#### Dependencies
- React 18, TypeScript, Vite for build tool.
